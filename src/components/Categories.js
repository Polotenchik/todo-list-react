...
class Categories extends Component {
    ...
    renderNodes = (data) => data.map(({ id, title, children }) => {
        ...
        return <TreeNode key={ id } title={ category } selected={ true }>
                    { children ? this.renderNodes(children, id) : ''}
                </TreeNode>
    });
    ...
    render() {
        ...
        return (
            <div>
                <CategoriesProgress />
                <Tree defaultExpandAll={ true } onSelected={ onSelect } selectedKeys={[selected.toString()]}>
                    { this.renderNodes(categories) }
                </Tree>
                <Button type='default' icon='plus' onClick={() => onAdd(0) } className='btn-add-category'>
                    Add category
                </Button>
            </div>
        );
